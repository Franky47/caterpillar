...
Tete.prototype.deplacerSelonCap = function () {
    this.x = this.x + (this.r * cos(this.cap));
    this.y = this.y + (this.r * sin(this.cap));
};
...
Tete.prototype.capOK = function (canvas) {
    ...
    var newx=this.x + (this.r * cos(this.cap));
    var newy=this.y + (this.r * sin(this.cap));
...
Chenille.prototype.dessiner = function () {
    ...
    anneaux=[];
    for (i=0; i<this.nbAnneaux;i++){
        var cetAnneau= new Anneau((this.canvas.width/2)-(i+1)*this.r, this.canvas.height/2, this.r);
        anneaux.push=cetAnneau;
        cetAnneau.dessiner(ctxt);
    };
    this.corps=anneaux;
